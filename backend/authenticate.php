...
if (isset($_POST['emplogin'])) {
    ...
    if($responseData->success){
      ...
      $query = "SELECT * FROM `employeedata` WHERE email='$email' LIMIT 1";
      ...
      if ($result) {
        ...
        //check email
        if ($email == $fetch_employee_result['email']) {
          #if password is matched then check for the password hash

          echo $_POST['pass'];
          echo $fetch_employee_result['pass'];
          if ($_POST['pass'] == $fetch_employee_result['pass']) {
            #on password match login the respected user
            session_start();
            
            $_SESSION['empfname'] = $fetch_employee_result['nom'];
            
            $_SESSION['emplname'] = $fetch_employee_result['prenom'];
            
            header("Location: ../employee/employee-dashboard.php");

          }else 
            echo "<script> alert('mot de pass'); window.location.href='../forms/employee-login.php';</script></script>";

        }else 
          echo "<script> alert('email'); window.location.href='../forms/employee-login.php';</script>"; 
